...
- **Spec Coverage Expansion (2026-06-06)**: Audited the 42 specs against the generation invariants and ZIP product contract and found that the artifacts the ZIP builder packages (`requirements/`, `project-management/`, `docs/`, `.github/`, `tools/`, `evals/`) had no generators, and several mandated capabilities (planning gate, architecture approval/versioning, session recovery, GitHub integration, scope-change protocol, marketing/onboarding) had no spec. Added specs 43–52 to close these gaps. Total spec count is now 52.
...
- **Session 2026-06-06**: Consolidated the full versioned research corpus into the master research files and propagated it through AGENTS.md, the six context files, and all feature specs. Then audited coverage and added specs 43–52 to generate every ZIP-contract artifact and implement the mandated planning-gate, architecture-approval, recovery, GitHub, and scope-change capabilities. The corpus now has 52 specs reflecting the diagram-first pipeline, four-layer architecture awareness, security/CI/CD/dependency/logging discipline, project-management and docs outputs, and the quality gate. No application code behavior changed; this was a documentation and specification synchronization and expansion pass.
