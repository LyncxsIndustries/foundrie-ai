...
- **Feature 62 invariant:** `security:deps` MUST remain `npm audit --audit-level=high` with **no** `--audit-level=none` and no advisory-ignore / suppress flags that hide high or critical CVEs. Context7 `/npm/cli` documents `--audit-level=none` as the official always-pass suppressor — it is forbidden here and in generated projects.
...
- Current Feature 62 transitive overrides (Context7-backed): `sharp@0.35.3` (exact; `allowScripts["sharp@0.35.3"]=true` must stay version-aligned) and `@opentelemetry/core@>=2.8.0 <3.0.0-0` (2.x-only floor under Trigger.dev’s OTel tree). `.npmrc` enables `strict-allow-scripts=true`. See `docs/SECURITY_SCRIPT_OVERRIDES.md` and `research/NPM_SECURITY_OVERRIDE_AUDIT.md`.
