...
- Foundrie uses a 2-role authorization model: **Owner** and **Collaborator**.
- Owner-only operations (project CRUD, settings, member management) use `requireProjectOwner()`.
- Shared operations (canvas editing, diagram CRUD, AI generation, ZIP download) use `requireProjectMember()` which authorizes both Owner and Collaborator.
- `getProjectRole()` returns `OWNER`, `COLLABORATOR`, or `null` for UI conditional rendering.
- Do not build PostgreSQL RLS, ABAC, audit logging, or hardware-key admin controls unless a later spec explicitly requires them.
...
- Read-heavy code imports `db`; writes and strong read-after-write paths import `db`.
...
- After pushing, let CodeRabbit review the GitHub PR and fix any additional findings before merging.
