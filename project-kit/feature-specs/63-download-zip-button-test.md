...
- `@testing-library/react` `16.3.2` + `@testing-library/jest-dom` / `user-event` (devDependencies)
...
## Files
CREATE: `components/project/DownloadZipButton.test.tsx` — Vitest + React Testing Library coverage for DownloadZipButton
...
- Expand coverage for edge error/retry paths if DownloadZipButton gains new states.
