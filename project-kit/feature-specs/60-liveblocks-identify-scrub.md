...
Feature 57 (PostHog `before_send` global scrub) is the primary privacy guard and already zeros `properties`, `$set`, and `$set_once` on every outbound browser event, including payloads emitted by `posthog.identify()`. This spec (60) scrubbing at the identify call site acts as defense-in-depth layer 2. Layer 1 = call-site scrub (Feature 60); Layer 2 = wire-payload scrub (Feature 57); Layer 3 = session-boundary reset (Feature 59). Even if layer 1 or 3 fails to execute on an edge case, layer 2 still removes all PII before the payload leaves the browser.
