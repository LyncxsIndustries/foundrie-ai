...
- Use `db` when creating `ConversationMessage` rows inside the Trigger.dev task. Use the `[conversationId, createdAt]` indexes to fetch recent messages.
...
- [ ] Conversation messages are stored in the structured relational `ConversationMessage` table.
