...
- When the exported project uses Prisma, generated architecture must follow the Prisma 7 standard: minimalist `datasource` in `schema.prisma` and database URLs configured via `prisma.config.ts`.
- When the exported project has authentication, explicitly separate authentication (Clerk/Auth provider) from authorization (application-level scoping).
...
- When the exported project supports collaboration, generate a 2-role authorization model: **Owner** (manages settings/members) and **Collaborator** (edits shared data), enforced via application-layer helpers rather than RLS.
- Do not generate enterprise RBAC, RLS, ABAC, audit logs, or custom admin architecture unless project requirements explicitly need them.
...
- Use `db` for large diagram/spec context reads when immediate read-after-write consistency is not required.
- Select only diagram metadata unless architecture generation explicitly needs React Flow JSON.
- Use `db` for the final context-file upsert.
