...
| Stale `@opentelemetry/core` (<2.8.0) in Trigger tree | Possible via transitive resolve (`2.7.1` seen pre-override) | Forced `>=2.8.0 <3.0.0-0` → `2.10.0` |
| Unbounded major float on OTel | `>=2.8.0` could resolve 3.x | Major-capped at `<3.0.0-0` |

## Decision: Exact sharp pin vs caret
Chose exact `0.35.3` over `^0.35.0` so CI and local installs resolve the security-hardened patch deterministically under a single concrete version key.

## Decision: OTel range vs exact
Chose `>=2.8.0 <3.0.0-0` (2.x-only floor) rather than pinning exact `2.10.0` or unbounded `>=2.8.0`, so Trigger.dev minor OTel bumps within 2.x can float while never resolving a future 3.x break without an explicit Feature update.

## Contract Sync Footprint (Feature 62)

| Artifact | Role |
|---|---|
| `package.json` / `package-lock.json` / `.npmrc` | Implementation |
