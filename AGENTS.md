...
4. `project-kit/context/project-overview.md` - product definition, users, flows, scope, and success criteria
5. `project-kit/context/architecture-context.md` - stack, system boundaries, AI orchestration, storage, database, APIs, and invariants
6. `project-kit/context/code-standards.md` - Foundrie-specific standards that extend the Artkins guide
7. `project-kit/context/ui-context.md` - dark workspace UI, diagram canvas, node/edge visual language, and interaction rules
8. `project-kit/context/ai-workflow-rules.md` - how agents must work, split tasks, use Context7, and keep docs synchronized
9. `project-kit/context/progress-tracker.md` - current state, next steps, open questions, and session notes
...
Use known library IDs from `project-kit/context/architecture-context.md` when available. Do not rely on memory for current APIs, setup, middleware, storage, Trigger.dev task syntax, Liveblocks patterns, React Flow APIs, Prisma migrations, or Next.js App Router behavior.

When working with the database, agents must use the project's defined NPM scripts (e.g., `npm run db:generate`, `npm run db:migrate`) to ensure Prisma runs correctly.
...
Feature specs are in `project-kit/feature-specs/`. Implement them in strict numeric order, one feature spec at a time.
...
4. Update `project-kit/context/progress-tracker.md`.
5. Run `coderabbit review --agent` locally before pushing. This is a mandatory pre-push gate.
6. Fix every CodeRabbit finding (critical and warning severity). Re-run `coderabbit review --agent` until only info-level or no findings remain.
7. Push the branch to GitHub.
8. Let CodeRabbit review the GitHub PR for any issues the local review may have missed.
9. Fix every GitHub CodeRabbit issue and push again. Repeat until there are no unresolved findings.
10. Mark the feature as done only after tests pass, build passes, and CodeRabbit has no unresolved findings.
11. Move to the next numbered spec.
