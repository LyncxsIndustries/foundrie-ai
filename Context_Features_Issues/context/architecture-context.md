...
# Optional Neon read replica pooled URL. Used for read-heavy traffic when available.
...
Do not build team workspaces, project owner/editor/viewer RBAC, a custom admin portal, PostgreSQL RLS, ABAC, audit logs, or hardware-key admin controls until a later feature explicitly requires them.
...
- Free plan defaults: 3 projects, 5 diagrams per project, 10 feature specs, ZIP export allowed, read-replica use disabled.
- Pro and Enterprise default to unlimited project/spec/diagram counts, ZIP export allowed, read-replica use allowed.
...
- `DATABASE_URL` is the Neon pooled runtime connection string and must use the `-pooler` endpoint.
- `DIRECT_URL` is the direct Neon connection string and is used only by Prisma CLI/migrations.
- `DATABASE_READ_REPLICA_URL` is optional and points to a Neon read replica for read-heavy traffic.
- Runtime code must not use the direct connection URL.
- If no read replica exists yet, read-heavy helpers fall back to `DATABASE_URL`.
...
- `dbWrite`: pooled primary connection, used for writes and strong read-after-write operations.
- `dbRead`: read-replica connection when configured, otherwise pooled primary, used for dashboards, ZIP assembly reads, artifact lists, and other large read paths.
- `db`: alias for `dbWrite` unless a file explicitly imports `dbRead`.
...
- Eventual consistency is acceptable only for progress UI, dashboard freshness, analytics, and replica-lag-tolerant reads.
...
6. Read-heavy flows use `dbRead` when they do not require immediate read-after-write consistency.
