...
- Implementation: `package.json` (`scripts.security:*`, `overrides`, `allowScripts`)
...
| `overrides.@opentelemetry/core` | (absent) | `>=2.8.0` (resolves to 2.10.0) |
...
- Keep `allowScripts` key version-matched so install scripts run for the overridden package.

### `@opentelemetry/core` (`/open-telemetry/opentelemetry-js`)
- Floor `>=2.8.0` under Trigger.dev’s OTel tree; registry latest at change time: `2.10.0`.

## Validation Checklist
1. `npm run security:deps` exits 0 with no high/critical advisories.
2. `npm ls sharp` shows `sharp@0.35.3 overridden`.
3. `npm ls @opentelemetry/core` shows `>=2.8.0` (currently `2.10.0`).
...
Exported TypeScript projects must ship the same `security:deps` / `security:all` scripts **without** ignore/suppress flags that hide high/critical CVEs, and must document any transitive `overrides` with Context7/npm evidence.
