...
|-------|--------|----------------------------------------|-------------------------------------------------------------------------------------------|
| 1     | 60     | `posthog.identify()` call site         | **IMPLEMENTED**: empty `email`/`name` strings — no Clerk PII at invoker time              |
...
7. **Server logger (Feature 61)** — `lib/posthog-server.ts` must use `logger.warn` / `logger.error` (no `console.*`). `capture` + `flush` must be try/caught so analytics failures never throw. Failure log is the fixed message `"PostHog capture error"` only — no `error.message`, no event name (`lib/logger.ts` has no scrub/redact). Unit coverage: `lib/posthog-server.test.ts`. Server payloads remain PII-free by construction (no browser `before_send` on Node). Product code must call `captureServerEvent` (not raw `posthog.capture`/`flush`).
